...
class SurfaceMaker:
    def __init__(self):
        #import all graphics
        for index, info in enumerate(walk('Breakout/graphics/blocks')):
            if index == 0:
                self.assets = {color:{} for color in info[1]}
            else:
                for image_name in info[2]:
                    color_type = list(self.assets.keys())[index-1]
                    full_path = 'Breakout/graphics/blocks' + f'/{color_type}/' + image_name 
    ...
    def get_surf(self, block_type, size):
        ...
        #bottom sides
        bot_width = size[0] - (sides['bottomleft'].get_width() + sides['bottomright'].get_width())
        scaled_bot_surf = pygame.transform.scale(sides['bottom'], (bot_width, sides['bottom'].get_height()))
        image.blit(scaled_bot_surf, (sides['topleft'].get_width(), size[1] - sides['topright'].get_height()))
        ...
        #return image to blocks and players
        return image